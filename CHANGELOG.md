...
All notable changes to Chason Design live here. The repo follows SemVer-ish discipline: experimental pre-releases iterate freely; stable releases lock the API.
...
- `chason-design` (install name `chason-design`) is now **v2 (experimental)**. The previous v1 is preserved as `chason-design-v1` (install name `chason-design-v1`).
- New `CHANGELOG.md` (this file).

---

## v2 (experimental) - the new default for `chason-design`

v2 (experimental) is a substantial rewrite of the original Chason Design. It keeps the dial-driven philosophy (`DESIGN_VARIANCE`, `MOTION_INTENSITY`, `VISUAL_DENSITY`) and adds structure, hard rules, and concrete implementation patterns the agent can actually follow.
...
- **§13 Out of Scope** - explicit list of what Chason Design is NOT for (dashboards, data tables, multi-step forms, code editors, native mobile, realtime collab UIs).
...
Pre-v2, the original Chason Design set the right direction but was easy for agents to skim past. Production testing showed the same Tells emerging across builds (em-dash everywhere, section-number eyebrows, "Quietly in use at", decorative dots, fake screenshots out of styled divs, broken GSAP scroll triggers).
...
## v1 - the original Chason Design

The original release. Dial-driven philosophy, anti-slop rules, reference vocabulary of pattern names. Preserved at `skills/chason-design-v1/` and installable as `chason-design-v1`.
